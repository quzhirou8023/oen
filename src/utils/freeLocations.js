const freeLocations = [
  {
    country: 'Netherlands',
    countryCode: 'nl',
    hosts: [
      { hostname: 'netherlands-free-https-1.techdatacdn.com', port: 443 },
      { hostname: 'netherlands-free-2.cloudspacecdn.site', port: 443 },
      { hostname: 'netherlands-https-3.networkcdn.site', port: 443 },
    ],
    latitude: 52.3878,
    longitude: 4.9302,
  },
  {
    country: 'Singapore',
    countryCode: 'sg',
    hosts: [
      { hostname: 'singapore-free-https-1.cloudspacecdn.site', port: 443 },
      { hostname: 'singapore-free-https-2.cloudnetcdn.site', port: 443 },
      { hostname: 'singapore-free-https-3.networkcdn.site', port: 443 },
    ],
    latitude: 1.3102,
    longitude: 103.6831,
    ratingLocked: true,
  },
  {
    country: 'USA West',
    countryCode: 'usw',
    hosts: [
      { hostname: 'usa-west-free-https-1.techdatacdn.com', port: 443 },
      { hostname: 'usa-west-free-2.cloudspacecdn.site', port: 443 },
      { hostname: 'usa-west-free-https-3.networkcdn.site', port: 443 },
    ],
    latitude: 34.0541,
    longitude: -118.2441,
  },
  {
    country: 'Australia',
    countryCode: 'au',
    isPremium: true,
  },
  {
    country: 'Brazil',
    countryCode: 'br',
    isPremium: true,
  },
  {
    country: 'Canada',
    countryCode: 'ca',
    isPremium: true,
  },
  {
    country: 'Chile',
    countryCode: 'cl',
    isPremium: true,
  },
  {
    country: 'France',
    countryCode: 'fr',
    isPremium: true,
  },
  {
    country: 'Germany',
    countryCode: 'de',
    isPremium: true,
  },
  {
    country: 'India',
    countryCode: 'in',
    isPremium: true,
  },
  {
    country: 'Japan',
    countryCode: 'jp',
    isPremium: true,
  },
  {
    country: 'South Korea',
    countryCode: 'kr',
    isPremium: true,
  },
  {
    country: 'Mexico',
    countryCode: 'mx',
    isPremium: true,
  },
  {
    country: 'Poland',
    countryCode: 'pl',
    isPremium: true,
  },
  {
    country: 'South Africa',
    countryCode: 'za',
    isPremium: true,
  },
  {
    country: 'Spain',
    countryCode: 'es',
    isPremium: true,
  },
  {
    country: 'Sweden',
    countryCode: 'se',
    isPremium: true,
  },
  {
    country: 'United Kingdom',
    countryCode: 'uk',
    isPremium: true,
  },
  {
    country: 'USA East',
    countryCode: 'use',
    isPremium: true,
  },
  {
    country: 'USA South',
    countryCode: 'uss',
    isPremium: true,
  },
]

export default freeLocations
